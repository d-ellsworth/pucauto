...
if __name__ == "__main__":
	...
	config = write_config(config,"min_value",raw_input("Minimum bundle value for trades: "))
	
	config = write_config(config,"hours_to_run",raw_input("Hours to run before restarting (0 for no restart): "))
	...
	if find_add_ons=="Y" or find_add_ons=="y":
		...
		config = write_config(config,"minutes_between_add_ons_check",raw_input("Minutes between addons check: "))
	else:
